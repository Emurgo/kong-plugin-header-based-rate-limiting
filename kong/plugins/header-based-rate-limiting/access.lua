...
function Access.execute(conf)
    ...
    local cache_key = kong.db.dao.header_based_rate_limits:cache_key(service_id, route_id, rate_limit_subject:encoded_identifier())
    local rate_limit_value = kong.cache:get(cache_key, nil, load_rate_limit_value, kong.db.dao, conf, rate_limit_subject)
